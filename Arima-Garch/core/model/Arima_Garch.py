...
class Arima_Garch:
    ...
    def plot_and_evaluate_forecast(self):
      ...
      plt.plot(self.results.index, self.results['Actual'], label='Actual')
      plt.plot(self.results.index, self.results['Forecast_Mean'], label='Forecast Mean')
...
class ArimaX_EGarch:
    ...
    def walk_forward_forecast(self, log_returns, tweets, window_size=7800, arima_order=(1,1,1), garch_order=(1,1)):
        """
        Perform walk-forward forecasting with a rolling window.
        ...
        try:
          for i in tqdm(range(window_size, n - 1), 'Iteration progress'):
              ...
              try:
                  # Fit ARIMA on rolling window
                  arima_model = SARIMAX(train, order=arima_order).fit()
